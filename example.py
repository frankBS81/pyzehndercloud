...
class ExampleAuth(AbstractAuth):
    ...
    def __init__(self, websession: ClientSession):
        ...
        if not self._tokens:
            print('Please provide a valid auth_token.json file. You can create one by running example_authenticate.py.')
            sys.exit(1)
    ...
    async def async_get_access_token(self) -> str:
        ...
        # }
        # async with self.websession.post(url, data=form) as response:
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        # await api.set_device_settings(devices[0], {'setVentilationPreset': {'value': 0}})

        # Sleep 5 seconds
        # await asyncio.sleep(5)

        # Set ventilation speed to low
        # await api.set_device_settings(devices[0], {'setVentilationPreset': {'value': 1}})
